...
export default function ClassifierEditPage({ params }: { params: { id: string } }) {
  ...
  const fetchData = async () => {
    try {
      const [classifier, allClassifiers] = await Promise.all([
        apiClient.get<Classifier>(`/classifiers/${params.id}`),
        apiClient.get<Classifier[]>('/classifiers'),
      ]);
      ...
      console.error('Failed to fetch classifier:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await apiClient.put(`/classifiers/${params.id}`, {
        ...formData,
        parentId: formData.parentId || null,
      });
      router.push('/reference/classifiers');
    } catch (error) {
      console.error('Failed to update classifier:', error);
    } finally {
      setSaving(false);
    }
  };
  ...
  const parentOptions = classifiers
    .filter((c) => c.type === formData.type && c.id !== params.id)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">Редактирование классификатора</h1>
