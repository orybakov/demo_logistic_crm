...
export default function EditRecipientPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [recipientResponse, filialsResponse] = await Promise.all([
          apiClient.get<Recipient>(`/recipients/${id}`),
          apiClient.get<{ data: Filial[] }>('/filials?isActive=true'),
        ]);

        setFormData({
          code: recipientResponse.code || '',
          name: recipientResponse.name,
          inn: recipientResponse.inn || '',
          kpp: recipientResponse.kpp || '',
          address: recipientResponse.address || '',
          city: recipientResponse.city || '',
          region: recipientResponse.region || '',
          postalCode: recipientResponse.postalCode || '',
          contactName: recipientResponse.contactName || '',
          contactPhone: recipientResponse.contactPhone || '',
          contactEmail: recipientResponse.contactEmail || '',
          notes: recipientResponse.notes || '',
          filialId: recipientResponse.filialId || '',
          isActive: recipientResponse.isActive,
        });
        setFilials(filialsResponse.data);
      } catch (error) {
        console.error('Failed to fetch recipient:', error);
        toast({
          title: 'Ошибка',
          description: 'Не удалось загрузить данные грузополучателя',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.put(`/recipients/${id}`, payload);
      toast({
        title: 'Успешно',
        description: 'Грузополучатель обновлен',
      });
      ...
      toast({
        title: 'Ошибка',
        description: 'Не удалось обновить грузополучателя',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Редактирование грузополучателя"
        breadcrumbs={[
