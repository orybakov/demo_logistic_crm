...
export default function WarehouseEditPage() {
  ...
  const fetchWarehouse = useCallback(async () => {
    try {
      const response = await apiClient.get<Warehouse>(`/warehouses/${params.id}`);
      ...
      setFormData({
        ...
        currentLoad: response.currentLoad?.toString() || '',
        workingHours: response.workingHours || '09:00-18:00',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await apiClient.put(`/warehouses/${params.id}`, payload);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <PageHeader
          title={`Редактирование склада: ${warehouse?.name || ''}`}
          breadcrumbs={[
